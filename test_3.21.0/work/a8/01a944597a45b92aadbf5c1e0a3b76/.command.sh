...
args_opt <- parse_args('--assay_names counts,counts_length_scaled,counts_scaled,lengths,tpm')
matrix_files <- as.list(strsplit('salmon.merged.gene_counts.tsv salmon.merged.gene_counts_length_scaled.tsv salmon.merged.gene_counts_scaled.tsv salmon.merged.gene_lengths.tsv salmon.merged.gene_tpm.tsv', ' ')[[1]])
...
if ('samplesheet_test.csv' != ''){
    coldata <- parse_metadata(
        metadata_path = 'samplesheet_test.csv',
...
if ('salmon.merged.gene' != 'null'){
    prefix = 'salmon.merged.gene'
} else if ('all_samples' != 'null'){
    prefix = 'all_samples'
}
...
writeLines(
    c(
        '"NFCORE_RNASEQ:RNASEQ:QUANTIFY_PSEUDO_ALIGNMENT:SE_GENE_UNIFIED":',
