...
bbsplit.sh \
    ...
    path=bbsplit \
    threads=4 \
    in=WT_REP1_trimmed_1_val_1.fq.gz in2=WT_REP1_trimmed_2_val_2.fq.gz \
    basename=WT_REP1_%_#.fastq.gz \
    refstats=WT_REP1.stats.txt \
    build=1 ambiguous2=all maxindel=150000 ow=f 2>| >(tee WT_REP1.log >&2)
