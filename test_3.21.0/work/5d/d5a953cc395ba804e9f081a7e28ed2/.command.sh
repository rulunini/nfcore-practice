...
pattern <- ifelse('salmon' == "kallisto", "abundance.tsv", "quant.sf")
fns <- list.files('quants', pattern = pattern, recursive = T, full.names = T)
names <- basename(dirname(fns))
names(fns) <- names
dropInfReps <- 'salmon' == "kallisto"

# Import transcript-level quantifications
txi <- tximport(fns, type = 'salmon', txOut = TRUE, dropInfReps = dropInfReps)

# Read transcript and sample data
transcript_info <- read_transcript_info('tx2gene.tsv')
...
writeLines(
    c(
        '"NFCORE_RNASEQ:RNASEQ:QUANTIFY_PSEUDO_ALIGNMENT:TXIMETA_TXIMPORT":',
