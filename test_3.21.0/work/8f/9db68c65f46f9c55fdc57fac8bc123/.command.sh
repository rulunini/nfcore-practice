...
args_opt <- parse_args('--assay_names counts,lengths,tpm')
matrix_files <- as.list(strsplit('salmon.merged.transcript_counts.tsv salmon.merged.transcript_lengths.tsv salmon.merged.transcript_tpm.tsv', ' ')[[1]])
...
if ('salmon.merged.transcript' != 'null'){
...
writeLines(
    c(
        '"NFCORE_RNASEQ:RNASEQ:QUANTIFY_PSEUDO_ALIGNMENT:SE_TRANSCRIPT_UNIFIED":',
